...
static const quint8 Crc8Table[256] ={ 0x00, 0x8D, 0x97, 0x1A, 0xA3, 0x2E, 0x34, 0xB9,
...
quint8 calcCrc8(const QByteArray &data) {
    quint8 crc = 0xFF;
    for (auto byte : data) {
        crc = Crc8Table[crc ^ static_cast<quint8>(byte)];
    }
    return crc;
}
...
quint16 calcCrc16(const QByteArray &data) {
    quint16 crc = 0xFFFF;
    for (auto byte : data) {
        crc = (crc << 8) ^ Crc16Table[(crc >> 8) ^ static_cast<quint8>(byte)];
    }
    return crc;
}
// В файле crcunit.cpp, внутри namespace CrcUnit

quint32 calcCrc32(const QByteArray &data) {
    quint32 crc = 0xFFFFFFFF;
    int size = data.size(); // Получаем размер данных
    // Получаем указатель на КОНСТАНТНЫЕ данные (безопаснее)
    const unsigned char *buffer = reinterpret_cast<const unsigned char*>(data.constData());

    // Используем цикл for с индексом, как более близкий аналог while(ASize--)
    for (int i = 0; i < size; ++i) {
        // Обращаемся к байту по указателю + смещение
...
quint32 calcCrc32_intermediate(const QByteArray &data) {
    ...
    return crc; // Возвращаем БЕЗ финального ~
}
}
