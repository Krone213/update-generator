...
/*
Имя файла: CrcUnit.cpp
автор: Гапонов Р.В.
Версия: 1.0.0
дата: 2012-10-24
описание: содержит функции для расчёта CRC
*/
...
// табличные данные для быстрого расчёта CRC8
...
/*! \fn unsigned char CalcCrc8 (void *ABuffer, int ASize);
 * \brief Ôóíêöèÿ ðàñ÷åòà CRC8 òàáëè÷íûì ìåòîäîì (ïîëèíîì 0x18D)
 * \param [in] ABuffer - óêàçàòåëü íà áóôåð ñ äàííûìè (áàéòîâîå âûðàâíèâàíèå)
 * \param [in] ASize - ðàçìåð äàííûõ, â áàéòàõ
 * \return Ôóíêöèÿ âîçâðàùàåò 8áèò çíà÷åíèå CRC8 äëÿ óêàçàííûõ äàííûõ
 */

unsigned char CalcCrc8 (void *ABuffer, int ASize)
{
unsigned char Crc8 = 0xFF;
unsigned char *Buffer = (unsigned char*) ABuffer;
while (ASize--) Crc8 = Crc8Table[Crc8 ^ *Buffer++];
return Crc8;
}
//================================================================================
...
unsigned char Crc8 = 0xFF;
int Index = 0;
while (ASize--) Crc8 = Crc8Table[Crc8 ^ ABuffer[Index++]];
return Crc8;
}
//================================================================================

// òàáëè÷íûå äàííûå äëÿ áûñòðîãî ðàñ÷åòà CRC16
...
/*! \fn unsigned short CalcCrc16 (void *ABuffer, int ASize);
 * \brief Ôóíêöèÿ ðàñ÷åòà CRC16 òàáëè÷íûì ìåòîäîì (ïîëèíîì CRC-16-CCITT 0x11021)
 * \param [in] ABuffer - óêàçàòåëü íà áóôåð ñ äàííûìè (áàéòîâîå âûðàâíèâàíèå)
 * \param [in] ASize - ðàçìåð äàííûõ, â áàéòàõ
 * \return Ôóíêöèÿ âîçâðàùàåò 16áèò çíà÷åíèå CRC16 äëÿ óêàçàííûõ äàííûõ
 */
...
unsigned char *Buffer = (unsigned char*) ABuffer;
while (ASize--) Crc16 = (Crc16 << 8) ^ Crc16Table[*Buffer++ ^ (Crc16 >> 8)];
return Crc16;
}
//================================================================================
...
unsigned short Crc16 = 0xFFFF;
int Index = 0;
while (ASize--) Crc16 = (Crc16 << 8) ^ Crc16Table[ABuffer[Index++] ^ (Crc16 >> 8)];
return Crc16;
}
//================================================================================

// òàáëè÷íûå äàííûå äëÿ áûñòðîãî ðàñ÷åòà CRC32
...
/*! \fn unsigned int CalcCrc32 (void *ABuffer, int ASize);
 * \brief Ôóíêöèÿ ðàñ÷åòà CRC32 òàáëè÷íûì ìåòîäîì (ïîëèíîì 0x04C11DB7)
 * \param [in] ABuffer - óêàçàòåëü íà áóôåð ñ äàííûìè (áàéòîâîå âûðàâíèâàíèå)
 * \param [in] ASize - ðàçìåð äàííûõ, â áàéòàõ
 * \return Ôóíêöèÿ âîçâðàùàåò 32áèò çíà÷åíèå CRC32 äëÿ óêàçàííûõ äàííûõ
 */
...
unsigned char *Buffer = (unsigned char*) ABuffer;
while (ASize--) Crc32 = (Crc32 >> 8) ^ Crc32Table[(*Buffer++ ^ Crc32) & 0xFF];
return (~Crc32);
}
//================================================================================
...
unsigned int Crc32 = 0xFFFFFFFF;
int Index = 0;
while (ASize--) Crc32 = (Crc32 >> 8) ^ Crc32Table[(ABuffer[Index++] ^ Crc32) & 0xFF];
return (~Crc32);
}
//================================================================================
